...
if (w < 756) {
  ...
  strText.style.fontSize = "18px";

  const handleChangeStr = (type) => {
    if (type == "prev") {
      timeLineNumb =
        timeLineNumb == 0 ? timelineList.length - 1 : timeLineNumb - 1;
    } else {
      timeLineNumb =
        timeLineNumb == timelineList.length - 1 ? 0 : timeLineNumb + 1;
    }
...
const figureList = [
  ...
  {
    key: "tessie",
    name: 'Katsuhiko "Tessie" Teshigawara',
    position: "0 -250px",
    img: "assets/image/tessie.jpg",
    intro:
...
figures.style.backgroundSize = "cover";
...
if (w > 756) figure.appendChild(img);
